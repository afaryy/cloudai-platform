...
| YY-50 private EKS protected CI path | Source implemented with staged zero-worker bootstrap, explicit runner/expanded endpoint phases, immutable amd64 private-ECR CPU smoke, fail-closed Job cleanup, bounded evidence, and a protected read-only pre-migration inventory that publishes Boolean-only evidence. Runtime inventory execution, AWS source-auth discovery, and all infrastructure/runtime validation remain pending | `.github/workflows/terraform-eks-private-sandbox.yml`, `.github/workflows/build-private-eks-cpu-smoke-image.yml`, `.github/workflows/terraform-runtime-inventory.yml`, `scripts/discover-terraform-runtime-inventory.sh`, `scripts/validate-private-eks-cpu-runtime.sh`, and `docs/solutions/eks-private-sandbox-runbook.md` |
...
| YY-52 VPC-connected CodeBuild runner foundation | Remote-state consumer, protected lifecycle workflow, and dedicated runner-state OIDC role are implemented; CloudFormation apply, Environment role handoff, and credential-free source validation are complete. Read-only CodeBuild GitHub source-auth discovery is implemented but must be applied to the bootstrap role and run before any runner plan; infrastructure and runtime validation remain pending | `providers/aws/infra/terraform/modules/private-runner/`, `providers/aws/infra/terraform/envs/eks-private-runner/`, `.github/workflows/terraform-eks-private-runner.yml`, `providers/aws/infra/bootstrap/github-oidc-terraform-backend.yaml`, and `docs/solutions/vpc-connected-runner-runbook.md` |
| YY-54 private platform teardown planning gate | Source implemented with exact dependency-order refusal, credential-free inspection, protected read-only state-presence planning, and no destructive execution path; no stop or teardown has been performed | `scripts/validate-private-eks-teardown-readiness.sh`, `scripts/tests/test-private-eks-teardown-readiness.sh`, `.github/workflows/private-eks-teardown-plan.yml`, and `docs/solutions/private-eks-layered-teardown-runbook.md` |
| Private-EKS GitHub environment readiness | Environment, required reviewer, main branch policy, backend/OIDC names, network bootstrap variables, and dedicated runner-state role handoff are configured; historical evidence is plan/source-only, while CodeBuild source-auth discovery, EKS/ARC values, and all infrastructure/runtime validation remain pending | `docs/solutions/private-eks-github-environment-readiness.md` |
