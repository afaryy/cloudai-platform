...
- **P7 AI Factory / LLMOps / GPU stretch:** retain the [AI Factory infrastructure lens](../architecture/ai-factory-infrastructure-lens.md), [AI Workload Operating Contract](./ai-workload-operating-contract.md), and [GPU workload readiness](../architecture/ai-factory-gpu-workload-readiness.md) as future design context; synthetic Agent/RAG, batch, fine-tuning, and distributed-training workload profiles now have metadata-only fixtures and contract tests, but there is still no current GPU, scheduler, or AI Factory implementation.
