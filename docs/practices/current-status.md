...
| YY-50 private EKS protected CI path | Source implemented with staged zero-worker bootstrap, explicit runner/expanded endpoint phases, immutable amd64 private-ECR CPU smoke, fail-closed Job cleanup, and bounded evidence; the dedicated runner Terraform role handoff and credential-free runner source validation are complete, while AWS source-auth discovery and all infrastructure/runtime validation remain pending | `.github/workflows/terraform-eks-private-sandbox.yml`, `.github/workflows/build-private-eks-cpu-smoke-image.yml`, `scripts/validate-private-eks-cpu-runtime.sh`, and `docs/solutions/eks-private-sandbox-runbook.md` |
