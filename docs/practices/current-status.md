...
| YY-50 private EKS protected CI path | Source implemented with staged zero-worker bootstrap, explicit runner/expanded endpoint phases, immutable amd64 private-ECR CPU smoke, fail-closed Job cleanup, and bounded evidence; all AWS runtime validation remains pending | `.github/workflows/terraform-eks-private-sandbox.yml`, `.github/workflows/build-private-eks-cpu-smoke-image.yml`, `scripts/validate-private-eks-cpu-runtime.sh`, and `docs/solutions/eks-private-sandbox-runbook.md` |
...
| Private-EKS GitHub environment readiness | Environment, required reviewer, main branch policy, backend/OIDC names, and network bootstrap variables configured; historical evidence is plan/source-only, while dedicated runner/EKS/ARC values and all AWS runtime validation remain pending | `docs/solutions/private-eks-github-environment-readiness.md` |
