# Practices

Reusable cross-layer engineering disciplines that apply across more than one
solution.

- [Agent Capability Governance](agent-capability-governance.md),
  [Agent Runtime Exploration](agent-runtime-exploration.md), and
  [AI Traffic Governance](ai-traffic-governance.md)
- [AI-Assisted DevSecOps](ai-assisted-devsecops-pattern.md) and
  [AI-Assisted Review Evidence](../evidence/ai-assisted-review-evidence.md)
- [AI Platform Security and Operations Controls](ai-platform-security-operations-controls.md)
- [AI FinOps](ai-finops.md), [Observability and Evaluation](observability.md),
  and [Operations Runbook](operations-runbook.md)
- [Responsible AI Checklist](responsible-ai-checklist.md) and
  [Secure AI Enablement](secure-ai-enablement.md)
- [AI Workload Operating Contract](ai-workload-operating-contract.md)
- [Agent Behavioural Reliability Gate](agent-behavioural-reliability-gate.md)
- [Current Status](current-status.md) and [Demo Script](demo-script.md)
