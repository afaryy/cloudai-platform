...
- **P8f Bedrock Guardrail Boundary:** Terraform-managed synthetic Guardrail/version, separate guarded-inference role, lifecycle-only bootstrap permission, and confirmation-gated workflow path; no live Guardrail apply or guarded call has been run for this slice
...
- **P8f operator gate:** apply the updated bootstrap permission boundary, review the Bedrock sandbox Terraform plan/apply, then set the separate guarded-smoke role ARN in `AWS_BEDROCK_GUARDRAIL_SMOKE_ROLE_TO_ASSUME` before explicitly dispatching the guarded smoke mode. Treat its result as attachment evidence only, not Guardrail-quality evidence.
- **P8 future provider boundary:** keep the validated IAM, synthetic smoke-test, opt-in adapter, and Guardrail attachment controls stable. Guardrail quality evaluation or AgentCore exploration needs a separate reviewed design and must not reuse P8f attachment evidence as proof of safety effectiveness.
