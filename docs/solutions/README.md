# Solutions

End-to-end portfolio case studies: the problem, architecture, technical
evidence, validation, trade-offs, and explicit limits.

- [Governed AI Gateway](genai-llm-gateway.md) and
  [Governed Model Access](governed-model-access.md)
- [Guardrails as a Service](guardrails-as-a-service.md)
- [AI Release Engineering on EKS](ai-release-engineering-on-eks.md) and
  [EKS Release Gates and Rollback](eks-release-gates-and-rollback.md)
- [Governed RAG Lifecycle](rag-knowledge-lifecycle.md)
- [Bounded Bedrock Sandbox](p8-real-bedrock-sandbox-design.md)
- [EKS Sandbox Design](p4b-real-eks-sandbox-design.md) and
  [Operator Runbook](p4b-eks-sandbox-operator-runbook.md)
- [EKS Prometheus and Grafana Demonstration](eks-prometheus-grafana-observability-demo.md)
- [Bedrock Access Readiness](p8a-bedrock-access-readiness.md) and
  [IAM Apply Readiness](p8b1-bedrock-iam-apply-readiness.md)
- [AgentCore Knowledge-Lookup Readiness](p8h-agentcore-knowledge-lookup-readiness.md)
- [AgentCore Synthetic Knowledge-Lookup Contract Pack](p8i-agentcore-synthetic-contract-pack.md)
- [AgentCore Governed RAG POC](../architecture/agentcore-governed-rag-poc.md),
  [data foundation](p8i-agentcore-rag-data-foundation.md),
  [key process record](p8i-agentcore-rag-key-process-record.md), and
  [POC runbook](agentcore-governed-rag-poc-runbook.md)
- [Private EKS GitHub Environment Readiness](private-eks-github-environment-readiness.md)
- [Private EKS Layered Teardown Planning Gate](private-eks-layered-teardown-runbook.md)
- [Solution Walkthrough](cloudai-platform-solution-walkthrough.md) and
  [Featured Solutions](featured-solutions.md)
