...
The next CI invoke run `32109877078` reached the provider and returned an AWS Marketplace subscription denial for the old Claude 3 Haiku model. The protected `aws-sandbox` variable `AGENTCORE_RAG_MODEL_ARN` was updated through the GitHub API to the already approved Claude Haiku 4.5 foundation model ARN (`anthropic.claude-haiku-4-5-20251001-v1:0`), which is also represented in the environment's approved model boundary. Terraform must now replace the application inference profile and Runtime configuration through the normal reviewed deploy plan/apply path; no console model-access change is being made.
