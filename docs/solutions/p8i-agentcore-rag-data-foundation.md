...
The repaired system-profile deployment succeeded through `deploy-apply` run `32141185366`. The subsequent Gateway invocation run `32143179818` returned an `answer` with one active synthetic citation and passed the response contract. The direct `RetrieveAndGenerate` preflight also reached Bedrock, but reported that the CI role lacked `bedrock:GetInferenceProfile` on the system profile. This is a verification-path IAM gap only; the Gateway path succeeded because the Runtime role already had the required profile read permission. The bootstrap policy is being amended to grant `GetInferenceProfile` alongside `InvokeModel` on that exact profile, with no broader model access.
