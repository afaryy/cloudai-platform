...
The next diagnostic identified `retrieval_invalid_configuration` as a Bedrock `ValidationException` from the Knowledge Base response-generation call. The first remediation created a single-region application inference profile from the approved foundation model through Terraform. That approach was later superseded when the approved Claude Haiku 4.5 foundation model reported that it does not support on-demand inference; the sandbox now uses the existing regional system inference profile directly.

A temporary read-only attempt to enumerate system-defined inference profiles through CI was intentionally denied because the bootstrap role does not include `bedrock:ListInferenceProfiles`; no AWS resource was changed. The approved system profile ARN is supplied through the protected GitHub environment and remains auditable without granting broad profile discovery.
...
The next change added `bedrock:InvokeModel` only for application inference profiles tagged `Project=cloudai-platform` and `Environment=agentcore-rag-sandbox`. This was a bounded diagnostic step; it did not resolve the provider-side model-access issue.

The policy was applied in Bootstrap run `32108066407`, then invoke run `32108142896` showed the next authorization boundary: Bedrock also checks `bedrock:InvokeModel` on the underlying approved Claude 3 Haiku foundation-model ARN. The next change grants that action only on the exact sandbox generation model ARN; it does not grant wildcard foundation-model access. If the next preflight succeeds, the remaining Gateway result will distinguish a genuine Knowledge Base/provider configuration issue from the Runtime's bounded error mapping.

The next CI invoke run `32109877078` reached the provider and returned an AWS Marketplace subscription denial for the old Claude 3 Haiku model. The protected `aws-sandbox` variable `AGENTCORE_RAG_MODEL_ARN` was updated through the GitHub API to the approved Claude Haiku 4.5 model boundary. The first deploy attempt (`32137817967`) then confirmed that this foundation model does not support creation of a new on-demand application profile. The implementation is being changed to use the existing approved regional system inference profile directly; the old application profile was already removed by the failed replacement attempt, and no production data or customer resources were involved.

The system-profile design grants `bedrock:InvokeModel` on the approved profile plus its two documented regional foundation-model destinations (`ap-southeast-2` and `ap-southeast-4`). This follows the Bedrock cross-Region inference permission model while keeping the policy restricted to one model family and the one approved profile.

The repaired system-profile deployment succeeded through `deploy-apply` run `32141185366`. The first Gateway invocation run `32143179818` returned an `answer` with one active synthetic citation and passed the response contract. Its direct `RetrieveAndGenerate` preflight reached Bedrock but reported that the CI role lacked `bedrock:GetInferenceProfile` on the system profile. This was a verification-path IAM gap only; the Gateway path succeeded because the Runtime role already had the required profile read permission. Bootstrap apply run `32144047070` added `GetInferenceProfile` alongside `InvokeModel` on that exact profile, with no broader model access.

The final validation run `32144157616` passed both paths: direct Bedrock `RetrieveAndGenerate` returned one citation, and the IAM-authenticated Gateway returned HTTP 200 with `outcome=answer`, an active synthetic citation, `citationPresent=true`, and `sourceLifecycle=active`. This closes the synthetic end-to-end RAG proof for the current profile architecture.
