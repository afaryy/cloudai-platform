...
The runtime has a single approved Bedrock Knowledge Base and model boundary. The protected environment supplies either an approved foundation-model ARN or an existing system inference-profile ARN; the current sandbox uses the approved regional system profile so response generation does not depend on on-demand support for the underlying model.
...
responses.
`AGENTCORE_RAG_MODEL_ARN` is the approved foundation-model or system
inference-profile ARN used directly for response generation.
